...
Study number     : _NA_
Sponsor          : _NA_
Program name     : avRaveDataDictionaryCompare.sas
Output           : RaveDataDictionaryCompareReport<dd-mm-yyyy>
Created on       : 
By               : SP.Standards
Modified         : 
Note             : 
...
%macro avRaveDataDictionaryCompare(pathInStandardDataDictionary=
	...
	%if ^%symglobl(studyno) %then %do;
		%put ERROR: 1/[AVANCE %sysfunc(datetime(), e8601dt.)] Macro variable studyno is not set in global scope;
		%put ERROR: 2/[AVANCE %sysfunc(datetime(), e8601dt.)] Macro &sysmacroname aborted;
		%return;
	%end;
	%if %sysevalf(%superq(studyno)=, boolean) or %sysevalf(%superq(client)=, boolean) %then %do;
		%put ERROR: 1/[AVANCE %sysfunc(datetime(), e8601dt.)] Macro variable client is not set in global scope;
		%put ERROR: 2/[AVANCE %sysfunc(datetime(), e8601dt.)] Macro &sysmacroname aborted;
		%return;
	%end;
	...
	%let file1Out2=FieldStd;
	%let file1Out3=CodeListStd;

	%let file2=&pathInCRFDataDictionary;
	%let file2Out1=FormCRF;
	%let file2Out2=FieldCRF;
	%let file2Out3=CodeListCRF;

	%let id1=OID DraftFormName;
	%let id1Summary=OID DraftFormName;
	...
	%let arrayColumns1=IsEproForm;
	%let relevantColumns1=OID -- SourceUrlId;
	%let sheet1=Forms;
	%let title1=Forms Level Comparison;

	%let id2=FormOID FieldOID;
	%let id2Summary=FormOID;
	%let id2columns1=id2Summary;
	%let id2columns2=id2;
	%let compareOut2=FieldProbs;
	%let out2=Field_verbose;
	%let out2Summary1=Field_brief_summary;
	%let out2Summary2=Field_extended_summary;
	%let arrayColumns2=VariableOID DataFormat DataDictionaryName UnitDictionaryName CodingDictionary ControlType FixedUnit DefaultValue	SASLabel SASFormat;
	%let relevantColumns2=FormOID -- AnalyteName_ValCol;
	%let sheet2=Fields;
	%let title2=Fields Level Comparison;

	%let id3=DataDictionaryName	CodedData;
	%let id3Summary=DataDictionaryName;
	%let id3columns1=id3Summary;
	%let id3columns2=id3;
	%let compareOut3=CodeListProbs;
	%let out3=CodeList_verbose;
	%let out3Summary1=CodeList_brief_summary;
	%let out3Summary2=CodeList_extended_summary;
	%let arrayColumns3=UserDataString;
	%let relevantColumns3=DataDictionaryName -- Specify;
	%let sheet3=DataDictionaryEntries;
	%let title3=DataDictionaryEntries Level Comparison;
	...
	%do i=1 %to 3;
		%do j=1 %to 2;
			...
			%let varExists = %sysfunc(varnum(&dsid, VariableOID));
			...
			data avgml.&&file&j.Out&i;
				set avgml.&&file&j.Out&i;
				keep &&relevantColumns&i.;
			run;

			/* Remove blank rows */
			proc sql;
				delete from avgml.&&file&j.Out&i where %scan(&&id&i, 1, %str( )) = "";
			quit;

			/* Remove label fields */
			%if &varExists %then %do;
				proc sql;
					delete from avgml.&&file&j.Out&i where VariableOID = "";
				quit;
			%end;

			/* Updated to simplify compare, allowing for variance in data dictionaries */
/*			%if &i = 1 %then %do;*/
/*				data avgml.&&file&j.Out&i (keep=form_name form_export_name form_external_id type object_type variable_name variable_export_name sas_label_export data_type units);*/
/*					set avgml.&&file&j.Out&i;*/
/*				run;*/
/*			%end;*/
	...
	ods escapechar='!';
	ods excel file="&pathOut.Rave_Data_Dictionary_Compare_Report_%left(%sysfunc(today(),date9.)).xlsx" options(sheet_name="Cover Page" embedded_footnotes='on' embedded_titles='on' frozen_headers="on" sheet_interval="none") style=BarrettsBlue;
		title1 j=c "Avance Clinical Pty.Ltd.";
		title2 j=c "PROTOCOL ID: &studyno";
		title3 j=c "CLIENT: &client";
		title4 j=c "Rave Data Dictionary Compare between &pathInStandardDataDictionary and &pathInCRFDataDictionary";
...
%mend avRaveDataDictionaryCompare;
